...
仓库的 `.github/workflows/docker-image.yml` 会在发布镜像前执行全部 Python 单元测试。推送形如 `v1.2.3` 的标签后，工作流发布：

```text
ghcr.io/<owner>/<repository>:server_1.2.3
ghcr.io/<owner>/<repository>:web_1.2.3
```

Fork 仓库首次发布前，需要按工作流现有约定配置名为 `TOKEN` 的 GitHub Actions Secret，并授予它写入该仓库 Packages 的权限；不要把令牌写进工作流文件。
